...
(or removed) from the :guilabel:`Purcahse` tab.
...
select the :guilabel:`Create [product name]` from the resulting drop-down menu, and manually add the
...
On the new |PO|, the :guilabel:`Order Deadline` field changes :guilabel:`Confirmation Date`, which
